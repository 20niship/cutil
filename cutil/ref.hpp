...
struct ControlBlock : public ControlBlockBase {
    ...
    static ControlBlock* create_empty() {
        return new ControlBlock(nullptr);
    }
};
...
class Ref {
    ...
    template <typename U>
    friend class enable_ref_from_this;
...
public:
    ...
    Ref(const Ref<U>& other) noexcept : ptr(other.ptr), cb(other.cb) {
        if (cb) {
            cb->strong_count.fetch_add(1, std::memory_order_acq_rel);
        }
    }
    ...
    template <typename U, typename = std::enable_if_t<std::is_convertible_v<U*, T*>>>
    Ref(Ref<U>&& other) noexcept : ptr(other.ptr), cb(other.cb) {
        other.ptr = nullptr;
        other.cb = nullptr;
    }
    ...
    template <typename U, typename = std::enable_if_t<std::is_convertible_v<U*, T*>>>
    explicit Ref(const WeakPtr<U>& weak) {
    ...
    template <typename Deleter>
    void reset(T* p, Deleter d) noexcept {
        if (p != ptr) {
            reset();
            if (p) {
                cb = new detail::ControlBlock<T, Deleter>(p, std::move(d));
    ...
    template <typename U>
    bool owner_before(const Ref<U>& other) const noexcept {
        return cb < other.cb;
    }
...
private:
    ...
    template <typename U>
    Ref(U* p, detail::ControlBlockBase* control_block, detail::PrivateConstructorTag) noexcept
...
public:
    ...
    // Copy constructor from convertible type
    template <typename U, typename = std::enable_if_t<std::is_convertible_v<U*, T*>>>
    WeakPtr(const WeakPtr<U>& other) noexcept : ptr(other.ptr), cb(other.cb) {
        if (cb) {
            cb->weak_count.fetch_add(1, std::memory_order_acq_rel);
        }
    }
    ...
    template <typename U, typename = std::enable_if_t<std::is_convertible_v<U*, T*>>>
    WeakPtr(const Ref<U>& ref) noexcept : ptr(ref.ptr), cb(ref.cb) {
        if (cb) {
            cb->weak_count.fetch_add(1, std::memory_order_acq_rel);
        }
    }
    ...
    // Assignment from Ref
    template <typename U, typename = std::enable_if_t<std::is_convertible_v<U*, T*>>>
    WeakPtr& operator=(const Ref<U>& ref) noexcept {
        reset();
        ptr = ref.ptr;
        cb = ref.cb;
        if (cb) {
...
private:
    ...
    template <typename U>
    WeakPtr(U* p, detail::ControlBlockBase* control_block) noexcept
        : ptr(p), cb(control_block) {}
};
...
protected:
    ...
    enable_ref_from_this& operator=(const enable_ref_from_this&) noexcept {
        return *this;
    }
...
bool operator==(const Ref<T>& a, const Ref<U>& b) noexcept {
    return a.get() == b.get();
}
...
template <typename T>
bool operator==(const Ref<T>& a, std::nullptr_t) noexcept {
    return a.get() == nullptr;
}

template <typename T>
bool operator==(std::nullptr_t, const Ref<T>& b) noexcept {
