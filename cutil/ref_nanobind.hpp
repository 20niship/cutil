...
template <typename T>
struct type_caster<cutil::Ref<T>> {
...
template <typename T>
struct type_caster<cutil::WeakPtr<T>> {
...
template <typename T>
struct is_holder<cutil::Ref<T>> : std::true_type {};

template <typename T>
struct holder_type<T, std::enable_if_t<std::is_same_v<cutil::Ref<T>, cutil::Ref<T>>>> {
    static_assert(std::is_base_of_v<cutil::enable_ref_from_this<T>, T>,
                  "Holder type Ref<T> requires T to inherit from enable_ref_from_this<T>");

    using type = cutil::Ref<T>;

    static constexpr auto init = [](cutil::Ref<T>& h, T *ptr) {
        h = cutil::make_ref<T>(*ptr);
    };
