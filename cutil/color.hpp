...
inline double RGB2V(const Vector3b& col) {
  double Value;
  const double R = col[0];
  const double G = col[1];
  const double B = col[2];
...
template <typename T = double> inline _Vec<T, 3> RGB2HSV(const Vector3b& col) {
  const double R = col[0];
  const double G = col[1];
  const double B = col[2];
  double MAX = std::max((std::max(R, G)), B);
...
template <typename T> inline Vector3b HSVtoRGB(const _Vec<T, 3>& hsv) {
