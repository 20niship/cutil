...
struct log_intermediate {
  ...
  log_intermediate(log_intermediate&& a) : buffer(move(a.buffer)), prefix(a.prefix), suffix(a.suffix), source(a.source), line(a.line), function(a.function), type(a.type) {}
  ...
  template <typename T> bool operator<<(const T& in) noexcept {
    try {
      ...
      buffer << "<<<<<exception on " << __PRETTY_FUNCTION__ << " what=" << e.what() << ">>>>>";
      return false;
    } catch(...) {
      buffer << "<<<<<exception on " << __PRETTY_FUNCTION__ << " uknown>>>>>";
      return false;
    }
  }
  ...
  ~log_intermediate() noexcept {
    try {
      ...
      std::stringstream s;
      s << prefix << "[ " << buf << "." << msec << "    " << type
        << "    \""
        // << source << "\" @ " << line
        << ' ' << function << "\t" << suffix << "\t" << buffer.str() << std::endl;
