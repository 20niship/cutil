...
template <typename T> struct Vector3 {
  ...
  template <typename U> Vector3 operator+=(const Vector3<U>& other) {
    x += other.x;
    y += other.y;
    z += other.z;
    return *this;
  ...
  template <typename U> Vector3 operator*=(const U& mul) {
    x = x * mul;
    y = y * mul;
    return *this;
  }
  template <typename U> Vector3 operator/=(const U& div) {
    x = x / div;
    y = y / div;
    return *this;
  }
  ...
  template <typename U> Vector3 operator+(const Vector3<U>& other) const {
    std::cout << "a" << std::endl;
  ...
  template <typename U> bool operator==(const Vector3<U>& other) const { return x == other.x && y == other.y && z == other.z; }
  template <typename U> bool operator!=(const Vector3<U>& other) const { return !(*this == other); }
