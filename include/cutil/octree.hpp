...
template<typename T, int DEPTH=10, int LEAF_MAX_N=5>
class Octree2{
public:
	class Node {};
	...
	struct Branch : public Node{
		Node *children[8];
		Branch() : children() {}
		void *operator new(size_t num_bytes, Pool<Branch> *mem) { assert(sizeof(Branch) == num_bytes); return mem->alloc_item(); }
		Node* operator[](unsigned i) const {  assert(i < 8);  return children[i]; }
	};
	...
	struct Leaves : public Node{
		...
		inline Leaves(const T *v) {values[0] = const_cast<T *>(v); size = 1;}
		void *operator new(size_t num_bytes, Pool<Leaves> *mem) { /* assert(this->size < LEAF_MAX_N); */ assert(sizeof(Leaves) == num_bytes); return mem->alloc_item(); }
		inline T* operator[](unsigned i) const {  assert(i < LEAF_MAX_N);  return values[i]; }
		inline bool hasCapacity(){return size < LEAF_MAX_N; }
		inline bool append(const T *v) {if(size >= LEAF_MAX_N){ return false; } values[size] = const_cast<T *>(v); size++;}
...
// private:
	...
	struct _int3{
		int x, y, z;
		_int3(){x = y = z = 0;}
		_int3(int x, int y, int z){this->x = x; this->y = y; this->z = z;}
		_int3 operator+=(const _int3& other) { x += other.x; y += other.y; z += other.z; return *this; }
		_int3 operator-=(const _int3& other) { x -= other.x; y -= other.y; z += other.z; return *this; }
	...
	struct Search{
		...
		inline void update_volume(Leaves *l, const uint64_t x, const uint64_t y, const uint64_t z){
			...
			const long dz = target_pos.z - z;
			const unsigned sq_distance = (dx * dx) + (dy * dy) + (dz * dz);
			if (sq_distance < r_sq){ nn_leaves.push_back(l); }
		}
		...
		inline bool check_branch(const int x, const int y, const int z, const int w) const {
			...
			// return true;   // 衝突！！
			return !(bbox_max.x < x || bbox_min.x > x+w || 
					 bbox_max.y < y || bbox_min.y > y+w || 
					 bbox_max.z < z || bbox_min.z > z+w);
		}
...
public: 
	...
	inline unsigned capacity() const {  auto w = resolution(); return w * w * w; }
	...
	inline bool hasNode(int x, int y, int z){
		...
		_int3 bin(
			(x - xmin) * resolution() / (xmax - xmin),
			(y - ymin) * resolution() / (ymax - ymin),
			(z - zmin) * resolution() / (zmax - zmin)
		);
		auto b = root();
